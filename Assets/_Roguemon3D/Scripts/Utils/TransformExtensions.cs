...
namespace UnityUtils {
    public static class TransformExtensions {
        ...
        public static bool HasLineOfSightTo(this Transform source, Transform target, LayerMask obstacleMask = default) {
            Vector3 directionToTarget = target.position - source.position;
            directionToTarget.y = 0f;
            float distanceToTarget = directionToTarget.magnitude + 0.01f; // Small buffer to avoid precision issues
            ...
            if (Physics.Raycast(source.position, directionToTarget.normalized, out RaycastHit hit, distanceToTarget, mask, QueryTriggerInteraction.Ignore)) {
